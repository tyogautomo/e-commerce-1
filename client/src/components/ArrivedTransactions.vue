<template>
  <div class="alltransactions">
    <table class="mt-4">
      ...
      <tr class="list" v-for="(trans, index) in allArrivedTransactions" :key="trans.id">
        <td>{{index + 1}}</td>
        <td>{{trans.id}}</td>
...
export default {
  data() {
    return {
      allArrivedTransactions: []
    };
  },
  methods: {
    getAllUsersArrivedTrasanctions() {
      axios({
        method: "get",
        url: `/carts/allusers/arrived`,
        headers: {
        ...
        .then(({ data }) => {
          ...
          this.allArrivedTransactions = newArrayOfTransactions;
        })
        .catch(err => {
          console.log(err);
        });
  ...
  created() {
    this.getAllUsersArrivedTrasanctions();
  }
};
</script>

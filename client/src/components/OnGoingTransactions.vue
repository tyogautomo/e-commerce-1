<template>
  <div class="alltransactions">
    <table class="mt-4">
      ...
      <tr class="list" v-for="(trans, index) in allOnGoingTransactions" :key="trans.id">
        <td>{{index + 1}}</td>
        <td>{{trans.id}}</td>
...
export default {
  data() {
    return {
      allOnGoingTransactions: []
    };
  },
  methods: {
    getAllUsersOnGoingTrasanctions() {
      axios({
        method: "get",
        url: `/carts/allusers/ongoing`,
        headers: {
        ...
        .then(({ data }) => {
          ...
          this.allOnGoingTransactions = newArrayOfTransactions;
        })
        .catch(err => {
          console.log(err);
        });
  ...
  created() {
    this.getAllUsersOnGoingTrasanctions();
  }
};
</script>
